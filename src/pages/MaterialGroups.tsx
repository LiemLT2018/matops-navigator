...
import type { ItemCategoryCatalog, ItemCategoryCreateBody } from '@/types/models';
...
export default function MaterialGroupsPage() {
  ...
  const [form, setForm] = useState<ItemCategoryCreateBody>({ code: '', name: '', status: 1 });

  const loadData = async () => {
    setLoading(true);
    try {
      const res = await itemCategoryService.list({ keyword: search || undefined, pageIndex: page, pageSize: 20 });
      setData(res.items);
  ...
  const handleCreate = () => {
    setEditingUuid(null);
    setForm({ code: '', name: '', status: 1 });
    setDialogOpen(true);
  };

  const handleEdit = (item: ItemCategoryCatalog) => {
  ...
  const handleSave = async () => {
    try {
      if (editingUuid) {
        await itemCategoryService.update(editingUuid, form);
        toast.success(t('common.edit') + ' ' + t('common.success'));
      } else {
        await itemCategoryService.create(form);
  ...
  return (
    <div className="space-y-4">
                ...
                <TableRow key={item.uuid} className="cursor-pointer hover:bg-muted/50">
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleEdit(item)}><Edit className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => handleDelete(item.uuid)}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingUuid ? t('common.edit') : t('common.create')} {t('materials.groups.title')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div><Label>{t('materials.groups.code')}</Label><Input value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} disabled={!!editingUuid} /></div>
            <div><Label>{t('materials.groups.name')}</Label><Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          </div>
          <DialogFooter>
