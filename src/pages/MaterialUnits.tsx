...
import { getMaterialUnits, type UnitOfMeasure } from '@/api/mockApi';
...
export default function MaterialUnitsPage() {
  ...
  useEffect(() => {
    getMaterialUnits().then(res => { setData(res.data); setLoading(false); });
  }, []);
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t('materials.units.symbol')}</TableHead>
                <TableHead>{t('materials.units.description')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
