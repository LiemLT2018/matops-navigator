...
import {
  BUSINESS_PARTNER_LIST_TYPES_CUSTOMER,
...
const emptyForm: BusinessPartnerCreateBody = {
  ...
  status: 1, type: BUSINESS_PARTNER_TYPE.CUSTOMER,
};
...
export default function CustomersPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const body: BusinessPartnerCreateBody = {
        ...
        name: form.name.trim(),
        type: uuidForUpdate ? (form.type ?? BUSINESS_PARTNER_TYPE.CUSTOMER) : BUSINESS_PARTNER_TYPE.CUSTOMER,
      };
      if (uuidForUpdate) {
        await businessPartnerService.update(uuidForUpdate, body);
        toast.success(t('errors.success'));
      } else {
        await businessPartnerService.create({ ...body, type: BUSINESS_PARTNER_TYPE.CUSTOMER });
  ...
  const openEdit = (item: BusinessPartnerDetail) => {
    ...
    setForm({
      ...
      code: item.code,
      name: item.name,
      type: item.type ?? BUSINESS_PARTNER_TYPE.CUSTOMER,
    ...
    <div className="space-y-4">
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEditingUuid(null); }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editingUuid ? t('partners.editCustomerTitle') : t('partners.customers.createTitle')}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-1">
            ...
            {editingUuid && (
              <div className="space-y-1.5">
                ...
                <Select
                  value={String(form.type ?? BUSINESS_PARTNER_TYPE.CUSTOMER)}
