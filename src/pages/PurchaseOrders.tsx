...
import { getPurchaseOrders, type PurchaseOrder } from '@/api/mockApi';
...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => { getPurchaseOrders().then(r => setData(r.data)); }, []);

  const filtered = data.filter(d =>
    !search || d.code.toLowerCase().includes(search.toLowerCase()) || d.supplier.toLowerCase().includes(search.toLowerCase())
  );
            ...
            <TableBody>
              {filtered.map(row => (
                <TableRow key={row.id} className="cursor-pointer hover:bg-muted/50">
                  ...
                  <TableCell className="font-mono text-sm text-muted-foreground">{row.prRef}</TableCell>
